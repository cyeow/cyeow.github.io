...
\usepackage{soul}
\usepackage{setspace}
\usepackage[a4paper,left=37mm,top=25mm,right=25mm,bottom=25mm, nohead]{geometry}
\usepackage{natbib}
\usepackage{tabularx}
\usepackage{colortbl}
\usepackage{titlesec}
...
\begin{table}[!h]
	...
	\label{table:geodata}
       	\csvreader[
      			tabular={|c c c c c c|},
            		table head=\hline Block &  Latitude & Longitude & Area & Population & Wastewater Production \\ $i$ & & & ($km^2$) & & $F_i$ ($m^3/day$)\\\hline,
            		table foot=\hline]
            		{data/blockgeo.csv}
...
\begin{table}[!h]
	...
	\label{table:cwdata}
       	\csvreader[
      			tabular={|c c c|},
            		table head=\hline Potential CW Site & Latitude & Longitude\\
							$j$				&		  &\\\hline,
            		table foot=\hline]
            		{data/cwgeo.csv}
            		{1=\j, 2=\lat, 3=\long}
...
\begin{table}[!h]
	...
	\label{table:polldata}
       	\csvreader[
      			tabular={|c c c c c|},
            		table head=\hline Indicator & \multicolumn{3}{c}{Influent concentration $\varepsilon^m$ ($mg/l$)} & Treatment target $\tau^m$ \\ $m$ & Minimum & Maximum & Average & ($mg/ml$)\\\hline,
            		table foot=\hline]
            		{data/pollutants.csv}
...
\begin{table}[!h]
	...
	\label{table:cwdata}
       	\csvreader[
      			tabular={|c c c c|},
            		table head=\hline Design Option & Flow Capacity & Area & Cost \\
							$k$			  & $Q_{k}$ ($m^3/day$)	& $A_k$ ($m^2$) & $c_{cw,k}$ (\$'000)\\\hline,
            		table foot=\hline]
            		{data/ccw.csv}
            		{1=\k, 2=\q, 3=\a, 4=\cost}
            		{\k & \q & \a & \cost}
...
\begin{table}[htb]
	...
	\caption{Model D optimal solution: Details of selected CWs}\label{table:resd}
       	\csvreader[
      			tabular={|c c c c|},
            		table head=\hline CW Site &  Design Option &  Flow Capacity &  Visual on \autoref{fig:networkd}\\\hline,
            		table foot=\hline]
            		{data/resultsd.csv}
...
%\begin{table}[htb]
%	\begin{center}
%	\caption[Short Caption for LoT]{Title of table}\label{table:summary}
%       	\csvreader[
%      			tabular={|c c c c c c|},
%            		table head=\hline Model & Average Success &  Average Shortfall &  Total Cost & Number of CWs built & Number of connections\\\hline,
%            		table foot=\hline]
%            		{data/resultssum.csv}
%            		{1=\model, 2=\avgsuccess, 3=\avgshortfall, 4=\cost, 5=\numcw, 6=\numcon}
%            		{\model & \avgsuccess & \avgshortfall & \cost & \numcw & \numcon}
%	\end{center}
%\end{table}
...
\begin{table}[htb]
	...
	\caption{Model S2 optimal solution: Details of selected CWs}\label{table:ress1}
       	\csvreader[
      			tabular={|c c c c|},
            		table head=\hline CW Site &  Design Option &  Flow Capacity &  Visual on \autoref{fig:networks1}\\\hline,
            		table foot=\hline]
            		{data/resultss1.csv}
            		{1=\j, 2=\k, 3=\q, 4=\visual}
            		{\j & \k & \q & \visual}
...
\begin{table}[htb]
	...
	\caption{Model S2 optimal solution: Details of selected CWs}\label{table:ress2}
       	\csvreader[
      			tabular={|c c c c|},
            		table head=\hline CW Site &  Design Option &  Flow Capacity &  Visual on \autoref{fig:networks2}\\\hline,
            		table foot=\hline]
            		{data/resultss2.csv}
            		{1=\j, 2=\k, 3=\q, 4=\visual}
            		{\j & \k & \q & \visual}
...
\section{Discussion}
%analysis
%usefulness of stochastic model
%worth the time?
%tie back into planning of network
%usefullness of planning using models beforehand
%
