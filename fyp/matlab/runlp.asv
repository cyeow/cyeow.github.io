...
%% run intlinprog
    ...
    x = reshape(res(1:end_x), size_i, size_j)
    y = reshape(res((end_x+1):end_y), size_i, size_j)
    z = reshape(res((end_y+1):end_z), size_j, size_k)
    s = reshape(res((end_z+1):numVars), size_i, size_j, size_k)
